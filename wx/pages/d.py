...
class Gallery(Base):
    ...
    def input_picture(self,url=url_pic):
        '''上传图片'''
        self.sendKeys(self.loc_8,url)

    def input_file(self,url2=url_file):
        '''上传附件'''
    ...
    def add_gallery(self,name="图片",itd="图片",cname="分类",url=url_pic,url2=url_file,t=1,p=1,y=1):
        '''添加商品'''
        time.sleep(2)
        ...
        self.input_shopname(name+self.timestr)
    ...
    def is_add_gallery_success(self):
        '''返回提示，最新商品的名称，价格'''
        time.sleep(2)
        ti = self.get_text(self.loc_tiishi)
        t = self.get_text(self.loc_14)
        p = self.get_text(self.loc_15)
        return ti,t,p

    def is_on_sale(self,is_on_sale=True):
        '''True:上架  False：下架'''
